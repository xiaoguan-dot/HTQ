...
由于 Google 搜索引擎被防火长城所屏蔽，国内大多数使用如百度、搜狗、360等搜索引擎，下文如遇见 Google （本文通常指搜索引擎而非 Google 公司）请按照 `这是一个搜索引擎` 理解。您可使用国内的搜索引擎对 Google 进行替换，但不保证搜索结果完全相同。

在国内的中文论坛里，请使用中文发问。

由于国内部分地区访问代码托管平台 Github 以阅读本文时可能遇见无法访问、卡顿等现象，故小关（本站管理，下同）将本文的页面托管在自己的域名下以便于阅读。

原文地址见上，转载的翻译后的文章仓库地址是：[Github:HandSonic/HTQ](https://github.com/HandSonic/HTQ)
...
小关额外添加的注意：在国内 Windows 使用较为广泛，虽然 Windows 系统确实比较烂，但部分专业软件与部分国内特供软件仅向用户提供了 Windows 版本，你可以到国内技术论坛做合理的发问。
...
> 小关补充国内特供版 问题：我怎么能盗取游戏账号/别人的社交账号（微信QQ等）/获取别人的个人信息（开盒）
